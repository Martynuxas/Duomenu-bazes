...
public class MainWindow extends BorderPane implements EventHandler<ActionEvent> {
    ...
    private void handleButtons(Object source) {
        if (source.equals(paneButtons.getButtons().get(0))) {
            ...
            find(paneParam1.getTfOfTable().get(6).getText());
        }
         else if (source.equals(paneButtons.getButtons().get(4))) {
            removeFromMyMap(paneParam1.getTfOfTable().get(6).getText());
        }
        else if (source.equals(paneButtons.getButtons().get(5))) {
            addBike(paneParam1.getTfOfTable().get(6).getText());
        }
    }
    ...
    private void mapGeneration(String filePath) {
        ...
        if (filePath == null) {
            Bike[] bikesArray = bikesGenerator.generateShuffleBikesAndIds(sizeOfGenSet, sizeOfInitialSubSet);
            for (Bike c : bikesArray) {
                map.put(
                        bikesGenerator.getBikeId(), //raktas
                        c);
                bCount ++;
            }
    ...
    private void mapPut() {
        Bike bike = bikesGenerator.getBike();
        map.put(
                bikesGenerator.getBikeId(), // Raktas
                bike);
        table.formTable(map.getMaxChainSize() * 2 + 1, colWidth);
        ...
        KsGui.oun(taEvents, bike, MESSAGES.getString("mapPut"));
    }
    ...
    private void find(String text) {
        Bike element = map.get(text);
        //Pvz:. DVI: kodas
        KsGui.oun(taEvents, (element == null ? "Tokio dviračio nėra" : element));
    }

    private void addBike(String text) {
        map.put("DVI" + String.format("%03d", bCount++), new Bike(text));
    }

    private void removeFromMyMap(String key) {
        Bike element = map.get(key);
        KsGui.oun(taEvents, (element == null ? "Tokio dviračio nėra" : element));
        map.remove(key);
    }
    ...
    private void createMap() {
        switch (cmbCollisionTypes.getSelectionModel().getSelectedIndex()) {
            case 0:
                map = new ParsableHashMap<>(String::new, Bike::new, initialCapacity, loadFactor, ht);
                break;
            // ...
    ...
    private void showMyMap() {
        table.getColumns().get(0).setMinWidth(300.0);
        table.getColumns().get(1).setMinWidth(600.0);
        table.getItems().clear();
        for (String bike : map.toArray()[0])
            table.getItems().add(new String[]{bike.split("=")[0], bike.split("=")[1]});
    }
